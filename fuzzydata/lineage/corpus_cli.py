...
"""
fuzzydata.lineage.corpus_cli
~~~~~~~~~~~~

Command-line entry point for corpus generation, installed as `fuzzydata-corpus`.

Lives inside the package rather than in scripts/ so that it actually ships: the legacy
setup.py `scripts=` list only carried the main fuzzydata CLI, so scripts/generate_corpus.py
was absent from the sdist and wheel entirely -- the release dry run caught it.
"""
